...
namespace MissionPlanner.Controls
{
    public partial class ConnectionControl : UserControl
    {
        ...
        private void Xbee_link_CheckedChanged(object sender, EventArgs e)
        {
            //xbee_protocol MissionPlanner.Comms
            if (xbee_link.Checked)
                MissionPlanner.Comms.SerialPort.use_xbee = true;
            else
                MissionPlanner.Comms.SerialPort.use_xbee = false;
        }
        public byte Selectedid;


        private void copter_select_SelectedValueClick(object sender, EventArgs e)
        {
            if (copter_select.SelectedItem != null)
            {
                Selectedid = byte.Parse((string)copter_select.SelectedItem);
                try //request data stream instantly
                {
                    MAVState MAV = MainV2.comPort.MAV;
                    MainV2.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.EXTENDED_STATUS, MAV.cs.ratestatus,
                        MAV.sysid, MAV.compid); // mode
                    MainV2.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.POSITION, MAV.cs.rateposition,
                        MAV.sysid, MAV.compid); // request gps
                    MainV2.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.EXTRA1, MAV.cs.rateattitude,
                        MAV.sysid, MAV.compid); // request attitude
                    MainV2.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.EXTRA2, MAV.cs.rateattitude,
                        MAV.sysid, MAV.compid); // request vfr
                    MainV2.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.EXTRA3, MAV.cs.ratesensors, MAV.sysid,
                        MAV.compid);// request extra stuff - tridge
                    MainV2.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.RAW_SENSORS, MAV.cs.ratesensors,
                        MAV.sysid, MAV.compid); // request raw sensor
                    MainV2.comPort.requestDatastream(MAVLink.MAV_DATA_STREAM.RC_CHANNELS, MAV.cs.raterc, MAV.sysid,
