...
namespace MissionPlanner
{
    public partial class MainV2 : Form
    {
        ...
        public ConcurrentDictionary<byte,DateTime> copter_id = new ConcurrentDictionary<byte, DateTime> ();
        public List<string> bind_combobox = new List<string>();
        string titlebar;
        ...
        {
            // MenuSimulation repalced by  //MenuSimulation
 ...
 //                   NativeMethods.ShowWindow(win, NativeMethods.SW_SHOWNORMAL); // hide window
        ...
        private void xbee_disconnect(object sender, EventArgs e)
        {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            //ui_modify
            ThreadPool.QueueUserWorkItem(BGcheck_multi_link);
        ...
        public class Group_states
        {
            public int sysid
            {
                get { return mav.sysid; }
            }
            public float alt
            {
                get { return mav.cs.alt; }
            }
            public float yaw
            {
                get { return (float)Math.Round(mav.cs.yaw,3); }
            }
            public string message
            {
                get { return mav.cs.message; }
            }
            MAVState mav;
            public Group_states(MAVState m)
            {
                mav = m;
            }
        };
        public BindingList<Group_states> group_state = new BindingList<Group_states>();
        private void BGcheck_multi_link(object state)
        {
            ...
            while (true)
            {
                if (comPort.BaseStream.IsOpen)
                {
                    ...
                    {
                        last_select = _connectionControl.copter_select.SelectedIndex;
                        _connectionControl.copter_select.Items.Clear();
                    foreach (var id in copter_id.Keys)
                    {
                        if (!(copter_id.TryGetValue(id, out t) && (t.AddSeconds(5) < DateTime.Now)))
                        {
                            _connectionControl.copter_select.Items.Add(id.ToString());
                        }
                    }
                    if (last_select < 0)
                        _connectionControl.copter_select.SelectedIndex = Math.Max(last_select, _connectionControl.copter_select.Items.Count - 1);
                    else
                        _connectionControl.copter_select.SelectedIndex= Math.Min(last_select, _connectionControl.copter_select.Items.Count-1);
                    
                    group_state.Clear();

                    foreach (var id in copter_id.Keys)
                    {
                        group_state.Add(new Group_states( comPort.MAVlist[id, 1]));
                    }
                    }));
                }
                Thread.Sleep(1000);
