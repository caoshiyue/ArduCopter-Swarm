...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        public FlightData()
        {
            ...
            CMB_modes_group.DataSource = Common.getModesList(MainV2.comPort.MAV.cs.firmware);
            CMB_modes_group.ValueMember = "Key";
            CMB_modes_group.DisplayMember = "Value";

            //default to auto
            CMB_modes_group.Text = "Guided";
            ...
            bindingSource_group_status.DataSource = MainV2.instance.group_state;        //rebind datasource
        }
                            ...
                            foreach (var port in MainV2.Comports.ToArray())
                            {
                                ...
                                foreach (var MAV in port.MAVlist)
                                {
                                    ...
                                    if (marker.Position.Lat == 0 && marker.Position.Lng == 0)
                                        continue;
                                    if (!check_exist_route(MAV))
                                    {
                                        var _route = Common.getMAVRoute(MAV);
                                        addMissionRouteroute(_route);
                                    }
        ...
        private void updateRoutePosition()
        {
            ...
            Invoke((MethodInvoker) delegate
            {
                ...
                foreach(var _r in routes.Routes)
                    gMapControl1.UpdateRouteLocalPosition(_r);
            });
        }
        private bool check_exist_route(MAVState mav)
        {
            int numTrackLength = Settings.Instance.GetInt32("NUM_tracklength");
            foreach (var r in routes.Routes)
            {
                if (r.id == mav.sysid)
                {
                    ...
                    r.Points.Add(_currentloc);
                    // maintain route history length
                        if (r.Points.Count > numTrackLength)
                        {
                            r.Points.RemoveRange(0,r.Points.Count - numTrackLength);
                        }
                    return true;
                }
            }
        ...
        private void addMissionRouteroute(GMapRoute newroute)
        {
            Invoke((MethodInvoker)delegate
            {
        ...
        private void gMapControl1_Click(object sender, EventArgs e)
        {
            ...
            foreach (var port in MainV2.Comports.ToArray())
                ...
                {
                    var marker = Common.getMAVMarker(MAV);
                    p = gMapControl1.FromLatLngToLocal(marker.Position);
                    if (MainV2._connectionControl.copter_select.Items.Contains(MAV.sysid.ToString()) && Math.Sqrt(Math.Abs(p.X - p2.X) * Math.Abs(p.X - p2.X) + Math.Abs(p.Y - p2.Y) * Math.Abs(p.Y - p2.Y)) < Math.Min(marker.Size.Width, marker.Size.Height) / 2)
                        //if(gMapControl1.IsMouseOverMarker)
                        MainV2._connectionControl.copter_select.SelectedItem = MAV.sysid.ToString();
                }
        }
        ...
        private void groupFlyToHereToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!MainV2.comPort.BaseStream.IsOpen)
            {
                CustomMessageBox.Show(Strings.PleaseConnect, Strings.ERROR);
            ...
            lock_formation_Click();
            Locationwp gotohere = new Locationwp();

            gotohere.id = (ushort)MAVLink.MAV_CMD.WAYPOINT;
            gotohere.alt = (float)modifyandSetAlt_group.Value;
            gotohere.lat = (MouseDownStart.Lat);
            gotohere.lng = (MouseDownStart.Lng);
            ...
            try
            {
                ...
                {
                    if (mav.sysid == 1)
                        MainV2.comPort.setGuidedModeWP(mav.sysid, mav.compid, gotohere);
                    else
                    {
                        Vector2d offset = formation[mav.sysid];
                        gotohere.lat = MouseDownStart.Lat+offset.X;
                        gotohere.lng = MouseDownStart.Lng+offset.Y;
                        MainV2.comPort.setGuidedModeWP(mav.sysid, mav.compid, gotohere);
                    }
                }
...
// square
        ...
        private void BUT_setmode_group_Click(object sender, EventArgs e)
        {
            if (MainV2.comPort.MAV.cs.failsafe)
            {
                if (CustomMessageBox.Show("You are in failsafe, are you sure?", "Failsafe", MessageBoxButtons.YesNo) != (int)DialogResult.Yes)
            ...
            foreach (var mav in MainV2.comPort.MAVlist)
            {
                MainV2.comPort.setMode(mav.sysid,mav.compid,CMB_modes_group.Text);
        ...
        private void CMB_modes_group_SelectedIndexChanged(object sender, EventArgs e)
        {
            CMB_modes_group.DataSource = Common.getModesList(MainV2.comPort.MAV.cs.firmware);
            CMB_modes_group.ValueMember = "Key";
            CMB_modes_group.DisplayMember = "Value";
        ...
        private void BUT_ARM_group_Click(object sender, EventArgs e)
        {
            if (!MainV2.comPort.BaseStream.IsOpen)
                return;
            ...
            try
            {
                ...
                foreach (var mav in MainV2.comPort.MAVlist)
                {
                    MainV2.comPort.setMode(mav.sysid, mav.compid, "GUIDED");
                    ans =MainV2.comPort.doARM(mav.sysid,mav.compid,true);
                    if (ans == false)
                        CustomMessageBox.Show(Strings.ErrorRejectedByMAV+ mav.sysid.ToString(), Strings.ERROR);
                }
            }
            catch
            {
        ...
        private void BUT_DISARM_group_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (var mav in MainV2.comPort.MAVlist)
                {
                    ans = MainV2.comPort.doARM(mav.sysid, mav.compid, false);
                    if (ans == false)
                        CustomMessageBox.Show(Strings.ErrorRejectedByMAV + mav.sysid.ToString(), Strings.ERROR);
                }
            }
        ...
        private void modifyandSetAlt_group_Click(object sender, EventArgs e)
        {
            int newalt = (int)modifyandSetAlt_group.Value;
            try
            {
                foreach(var mav in MainV2.comPort.MAVlist)
                MainV2.comPort.setNewWPAlt(mav.sysid,mav.compid,new Locationwp { alt = newalt / CurrentState.multiplierdist });
            }
            catch
            {
                CustomMessageBox.Show(Strings.ErrorCommunicating, Strings.ERROR);
        ...
        private void modifyandSetSpeed_group_Click(object sender, EventArgs e)
        {
            // QUAD
            if (MainV2.comPort.MAV.param.ContainsKey("WP_SPEED_MAX"))
            {
        ...
        private void BUTtakeoff_Click(object sender, EventArgs e)
        {
            ...
            {
                //flyToHereAltToolStripMenuItem_Click(null, null);
                int alt = (int)modifyandSetAlt_group.Value;
                try
                {
                    foreach (var mav in MainV2.comPort.MAVlist)
                    {
                        MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_HOME, 0, 0, 0, 0, (float)mav.cs.lat,(float)mav.cs.lng, mav.cs.alt);
                        MainV2.comPort.doCommand(mav.sysid, mav.compid,MAVLink.MAV_CMD.TAKEOFF, 0, 0, 0, 0, 0, 0, alt);
                    }
                }
                catch
                {
        ...
        private void BUTland_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                foreach (var mav in MainV2.comPort.MAVlist)
                {
                    MainV2.comPort.doCommand(mav.sysid, mav.compid, MAVLink.MAV_CMD.LAND, 0, 0, 0, 0, 0, 0, 0);
                }
            }
            catch
            {
        ...
        internal Dictionary<int ,Vector2d> formation=new Dictionary<int, Vector2d>();
        private void lock_formation_Click()
        {
            if(formation!=null)
                formation.Clear();
            ...
            {
                CustomMessageBox.Show(Strings.CommandFailed, "can't find origin");
                return;
            }
            foreach (var mav in MainV2.comPort.MAVlist)
            {
                Vector2d offset = new Vector2d(mav.cs.lat, mav.cs.lng)- origin;
                formation.Add(mav.sysid, offset);//纬 经
            }
        }
    }
}
