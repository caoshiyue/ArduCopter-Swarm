...
class ChibiOS::XBee_UARTDriver : public ChibiOS::UARTDriver 
{
public:
    XBee_UARTDriver(uint8_t serial_num):UARTDriver(serial_num){}
/// override function 
    size_t write(uint8_t c) override;
    size_t write(const uint8_t *buffer, size_t size, uint8_t chan) override;
    uint32_t available() override;
    int16_t read() override;
///
    uint16_t targ_add= 0xdfdf;
private:
/// copy function in ChibiOS::UARTDriver 
    size_t base_write(uint8_t c);
    size_t base_write(const uint8_t *buffer, size_t size);
    uint32_t base_available();
    int16_t base_read();
/// xbee protocol process 
