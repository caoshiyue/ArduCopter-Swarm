...
int16_t XBee_UARTDriver::base_read()
{
    if (lock_read_key != 0 || _uart_owner_thd != chThdGetSelfX())
    {
        return -1;
...
size_t XBee_UARTDriver::base_write(uint8_t c)
{
    if (lock_write_key != 0 || !_write_mutex.take_nonblocking())
    {
        return 0;
...
size_t XBee_UARTDriver::base_write(const uint8_t *buffer, size_t size)
{
    if (!_initialised || lock_write_key != 0)
    {
        return 0;
...
uint32_t XBee_UARTDriver::base_available()
{
    if (!_initialised || lock_read_key)
    {
        return 0;
...
uint16_t XBee_UARTDriver::pack(const char *data, uint16_t lenth) // lenth of data
{
    uint16_t pack_lenth;
    uint8_t i = 0;
    uint8_t check;
...
size_t XBee_UARTDriver::write(uint8_t c)
{
    return base_write(c);
}

size_t XBee_UARTDriver::write(const uint8_t *buffer, size_t size, uint8_t chan)
{
    uint16_t lenth = 0;
    if (chan == 2)
        targ_add = 0xdfdf; //gcs
...
int16_t XBee_UARTDriver::read()
{
    return decode();
}

uint32_t XBee_UARTDriver::data_available()
{
    if (!operating)
    {
        uint8_t byte[8] = {0};
        while (base_available() > 8)
        {
            byte[0] = base_read();
            if (byte[0] == 0x7E)
            {
...
uint32_t XBee_UARTDriver::available()
{
    return data_available();
}

#endif //  XBEE_TELEM == ENABLED
