						    ...
						    uint8_t min_length, uint8_t length, uint8_t crc_extra)
{
	...
	uint8_t max_packet[sizeof(mavlink_message_t)+MAVLINK_SIGNATURE_BLOCK_LEN+1]={0};
	memcpy(max_packet,buf,header_len+1);
	memcpy(&max_packet[header_len+1],packet,length);
	memcpy(&max_packet[header_len+1+length],ck,2);
	memcpy(&max_packet[header_len+3+length],signature,signature_len);
...
/*
	...
	*/
	if(signature_len == 0)
		_mavlink_send_uart(chan,(const char *)max_packet,header_len+3+length);
	else
		_mavlink_send_uart(chan,(const char *)max_packet,header_len+3+length+signature_len);
...
MAVLINK_HELPER void _mavlink_resend_uart(mavlink_channel_t chan, const mavlink_message_t *msg)
{
	...
	// XXX use the right sequence here
        ...
        if (msg->magic == MAVLINK_STX_MAVLINK1) {
			...
			memcpy(max_packet,buf,header_len);
        } else {
	...
	//_mavlink_send_uart(chan, (const char *)ck, 2);
	memcpy(&max_packet[header_len],_MAV_PAYLOAD(msg),msg->len);
	memcpy(&max_packet[header_len+msg->len],ck,2);
	memcpy(&max_packet[header_len+2+msg->len],msg->signature,MAVLINK_SIGNATURE_BLOCK_LEN);
    if(signature_len == 0)
	   _mavlink_send_uart(chan,(const char *)max_packet,header_len+2+msg->len);
	else
	   _mavlink_send_uart(chan,(const char *)max_packet,header_len+2+msg->len+MAVLINK_SIGNATURE_BLOCK_LEN);
