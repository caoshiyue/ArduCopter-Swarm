...
namespace MissionPlanner.Comms
{
    ...
    public class SerialPort : System.IO.Ports.SerialPort, ICommsSerial
    {
        ...
        */
...
#if XBEE_PROTOCOL
        public static bool use_xbee = true;
        public static UInt64 remote_add = 0;
        private bool operating = false;
        private UInt16 datalenth = 0;
        public new int Read(byte[] buffer, int offset, int count)
        {
            if (use_xbee)
            {
                DateTime to = DateTime.Now.AddMilliseconds(300);
                int toread = count;
                int pos = offset;
                while (true)
                {
                    int read = Read_without_timeout(buffer, pos, toread);
        ...
        private int Read_without_timeout(byte[] buffer, int offset, int count)
        {
            try
            {
                if (!operating)
                {
                    byte[] head_buf = new byte[15];
                    while (BytesToRead > 15)
                    {
                        head_buf[0] = (byte)base.ReadByte();
                        if (head_buf[0] == 0x7E)
                        {
                            base.Read(head_buf, 1, 3);
                            if (head_buf[3] == 0x90)
                            {
                                base.Read(head_buf, 4, 11);
                                datalenth = (UInt16)(head_buf[1] * 256 + head_buf[2] - 12);
                                remote_add = 0;
                                for (int x = 4; x < 12; x++)
                                    remote_add += (UInt64)head_buf[x] << ((11 - x) * 8);
                                operating = true;
                                break;
                            }
                        }
                ...
                if (operating)
                {
                    var toread = Math.Min(Math.Min(count, BytesToRead), datalenth);
                    var haveread = (ushort)base.Read(buffer, offset, toread);
                    datalenth -= haveread;
                    if (datalenth == 0)
                    {
                        operating = false;
        ...
        {
            Byte[] b = Encoding.Default.GetBytes(buffer);
