...
4. Try a diffrent usb port\n\n" +
...
#if XBEE_PROTOCOL
        public UInt64 targ_add = 0;//0x0013A2004178E980;
        public Dictionary<int, UInt64> targ_add_dic = new Dictionary<int, ulong>();
        private void set_remote_add(int sysid)
        {
            targ_add_dic[sysid] = SerialPort.remote_add;
        }
#endif
        ...
        internal void generatePacket(int messageType, object indata, int sysid, int compid, bool forcemavlink2 = false, bool forcesigning = false)
        {
            ...
            lock (objlock)
            {
                ...
                {
#if XBEE_PROTOCOL
                    if (MainV2._connectionControl.xbee_link.Checked)
                    {
                        byte[] pack_buf = new byte[320];
                        const int XBEEMAXDATA = 250;
                        UInt16 pack_lenth;
                        int lenth = packet.Length;
                        int n = 0;
                        if (targ_add_dic.ContainsKey(sysid))
                            targ_add = targ_add_dic[sysid];
                        for (n = 0; n < ((lenth - 1) / XBEEMAXDATA) + 1; n++)
                        {
                            pack_lenth = 0;
                            if ((lenth - XBEEMAXDATA * n) >= XBEEMAXDATA)
                                pack_lenth = XBEEMAXDATA;
                            else
                                pack_lenth = (UInt16)(lenth - XBEEMAXDATA * n);
                            pack_buf[0 + (XBEEMAXDATA + 18) * n] = 0x7e;
                            pack_lenth += 14;
                            pack_buf[1 + (XBEEMAXDATA + 18) * n] = (byte)(pack_lenth >> 8);
                            pack_buf[2 + (XBEEMAXDATA + 18) * n] = (byte)(pack_lenth & 0xff);
                            pack_buf[3 + (XBEEMAXDATA + 18) * n] = 0x10;
                            pack_buf[4 + (XBEEMAXDATA + 18) * n] = 0;
                            for (int x = 0; x < 8; x++)
                                pack_buf[5 + (XBEEMAXDATA + 18) * n + x] = (byte)((targ_add >> ((7 - x) * 8)) & 0xff);
                            pack_buf[13 + (XBEEMAXDATA + 18) * n] = 0xff;
                            pack_buf[14 + (XBEEMAXDATA + 18) * n] = 0xfe;
                            pack_buf[15 + (XBEEMAXDATA + 18) * n] = 0;
                            pack_buf[16 + (XBEEMAXDATA + 18) * n] = 0;
                            Array.Copy(packet, 0, pack_buf, (17 + (XBEEMAXDATA + 18) * n), pack_lenth - 14);
                            byte check = 0;
                            for (int u = 3; u < pack_lenth + 3; u++)
                                check += pack_buf[u + (XBEEMAXDATA + 18) * n];
                            pack_buf[pack_lenth + 3 + (XBEEMAXDATA + 18) * n] = (byte)(0xFF - check);
                        }
                        BaseStream.Write(pack_buf, 0, lenth + 18 * n);
                        _bytesSentSubj.OnNext(lenth + 18 * n);
                    }
                    else
                    {
        ...
        public void generatePacket_group(int messageType, object indata, int sysid, int compid)
        {
            if (!BaseStream.IsOpen)
            {
                return;
            ...
            lock (objlock)
            {
                ...
                // are we mavlink2 enabled for this sysid/compid
                if (!MAVlist[sysid, compid].mavlinkv2 && messageType < 256 )
                {
                    var info = MAVLINK_MESSAGE_INFOS.SingleOrDefault(p => p.msgid == messageType);
                    if (data.Length != info.minlength)
                    {
        ...
        */
        ...
        {
            return doCommand(sysid, compid, MAV_CMD.COMPONENT_ARM_DISARM, armit ? 1 : 0, 21196, 0, 0, 0, 0, 0);// 21196?
        }
        ...
        public MAVLinkMessage readPacket()
        {
            ...
            lock (readlock)
            {
                ...
                set_remote_add(buffer[3]);      //before next read, get remote add and match sysid
            ...
            try
            {
                if ((message.header == 'U' || message.header == 0xfe || message.header == 0xfd) && buffer.Length >= message.payloadlength)
                {
                    ...
                    lock (objlock)
                    {
                        ...
                        if (msgid == (byte)MAVLINK_MSG_ID.GLOBAL_POSITION_INT|| msgid == (byte)MAVLINK_MSG_ID.ATTITUDE|| msgid == (byte)MAVLINK_MSG_ID.HEARTBEAT)//only for update sysid
                        {
                            DateTime _t = DateTime.Now;
                            MainV2.instance.copter_id.AddOrUpdate(sysid, _t, (key, oldvalue) => _t);
                        }
                        ...
                        if (msgid == (byte)MAVLINK_MSG_ID.HEARTBEAT)
                    {
                        ...
                        if (hb.type != (byte) MAV_TYPE.GCS)
                        {
                            ...
                            {
                                _sysidcurrent = MainV2._connectionControl.Selectedid;
                                compidcurrent = compid;
                            }
                        }
                    }
