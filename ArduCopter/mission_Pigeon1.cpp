...
float Mission_pigeon::force_target(bool x) //x==true
{
    ...
    status_int[0]=(int8_t)dp.x;
    status_int[1]=(int8_t)dp.y;	
    status_int[2]=(int8_t)(mav[1].Pxyz.alt/100);
    if (up == 0)
        return 0;
    if (x)
    {
        float distance = disx[1] * cos(mav[1].yaw*3.1415926/180.0f) - disy[1] * sin(mav[1].yaw*3.1415926/180.0f); //x
        return (pow(1.2, dp.x - distance) - pow(1.2, -(dp.x - distance))) * k2x1 + atan(dp.x - distance) / 3.1415 * 3 - repulsion(1, 1.5, 1, true);
    }
    else
    {
        float distance = disx[1] * sin(mav[1].yaw*3.1415926/180.0f) + disy[1] * cos(mav[1].yaw*3.1415926/180.0f); //y
